"""
Utility for importing history traffic file into database
"""
import sys
import django
import os
...
from slice_by_region import TrafficDataSlicer

class TrafficDBImporter(object):
...
class AnotherAutoTrafficImporter(AutoTrafficImporter):
    def preprocess(self,txt):
        ...
        for line in lines:
            ...
            speed = float(line[3])
            cat = line[2]
            to_print.append("{},{}".format(id,speed))
            seg = RoadSeg.objects.get(id=id)
            if (id == 107001426):
                a = id
            to_return.append(
                {
...
if __name__ == "__main__":
    ...
    for name in names:
        importer.import_data(src_txt_name="D:/fyp/data_repo/{}.txt".format(name),traffic_name=name)
